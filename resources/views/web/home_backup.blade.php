...
@section('content')
        ...
        <div class="hero-dots" style="position: absolute; bottom: 30px; left: 50%; transform: translateX(-50%); display: flex; gap: 12px; z-index: 10;">
            @foreach($heroSlides as $index => $slide)
                <button class="hero-dot {{ $index === 0 ? 'active' : '' }}" 
                        ...
                        data-slide="{{ $index }}"
                        style="width: 12px; height: 12px; border-radius: 50%; border: 2px solid white; background: {{ $index === 0 ? 'white' : 'transparent' }}; cursor: pointer; transition: all 0.3s; opacity: {{ $index === 0 ? '1' : '0.6' }};">
                </button>
            @endforeach
        </div>
        @endif
    ...
    <section class="skc-section" style="background: #f8f9fa; padding: 80px 0;">
        <div class="skc-container">
            <div class="skc-section-header" style="text-align: left;">
                <h2 class="skc-section-title">Limited Time Offer - Special Bundle Deals</h2>
            </div>
                    ...
                    <style>
                        ...
                        <div style="flex: 1.4; position: relative; background: #f8f9fa; display: flex; align-items: center; justify-content: center; padding: 40px;">
                            @if($featuredBundle->image_path)
                                <img src="{{ Storage::url($featuredBundle->image_path) }}" alt="{{ $featuredBundle->name }}" 
                                     style="width: 100%; height: 100%; max-width: 450px; max-height: 450px; object-fit: cover; border-radius: 12px;">
                            @else
                                <div style="width: 300px; height: 300px; background: linear-gradient(135deg, #e3f2fd 0%, #bbdefb 100%); border-radius: 12px; display: flex; align-items: center; justify-content: center; position: relative;">
                                    <span style="font-size: 120px;">🎁</span>
                                    <!-- Discount Badge -->
                                    @if($featuredBundle->savings_percentage > 0)
                                    <div style="position: absolute; top: 15px; right: 15px; background: #007bff; color: white; padding: 8px 16px; border-radius: 20px; font-size: 14px; font-weight: 600;">
                        ...
                        <div style="flex: 1.6; padding: 40px; display: flex; flex-direction: column; justify-content: space-between;">
                            ...
                            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px;">
                                <div style="background: #007bff; color: white; padding: 8px 16px; border-radius: 20px; font-size: 12px; font-weight: 600; display: flex; align-items: center; gap: 6px;">
                            ...
                            <!-- Add to Cart Section -->
                            <div class="add-to-cart-section" style="margin-bottom: 20px;">
                                <button onclick="addBundleToCart({{ $featuredBundle->id }})" 
                                        style="background: #007bff; color: white; padding: 16px 32px; border: none; border-radius: 12px; font-size: 16px; font-weight: 600; cursor: pointer; transition: all 0.3s; display: flex; align-items: center; gap: 8px; justify-content: center; width: 100%; max-width: 300px;"
                            ...
                            <div class="delivery-info" style="display: flex; gap: 30px; padding-top: 20px; border-top: 1px solid #f0f0f0;">
                               ...
                               <!-- <div style="display: flex; align-items: center; gap: 8px;">
                                    <i class="fas fa-clock" style="color: #ffc107; font-size: 16px;"></i>
                                    <span style="font-size: 14px; color: #666;">24h dispatch</span>
                                </div>-->
                                <div style="display: flex; align-items: center; gap: 8px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 30px; margin-top: 50px;">
                @foreach($bundles->skip(1) as $bundle)
                    <div style="background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.08); transition: transform 0.3s, box-shadow 0.3s; position: relative;" 
                        ...
                        <div style="position: absolute; top: 15px; right: 15px; background: #007bff; color: white; padding: 5px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; z-index: 10;">
                        ...
                        <div style="height: 200px; background: #f8f8f8; position: relative; overflow: hidden;">
                            ...
                            @else
                                <div style="display: flex; align-items: center; justify-content: center; height: 100%; background: linear-gradient(135deg, #007bff 0%, #0056b3 100%);">
                        ...
                        <div style="padding: 20px;">
                            ...
                            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px;">
                                <div>
                                    ...
                                    @endif
                                    <div style="font-size: 24px; font-weight: 700; color: #007bff;">₹{{ number_format($bundle->price, 2) }}</div>
                                </div>
                                @if($bundle->savings > 0)
                            ...
                            <button onclick="addBundleToCart({{ $bundle->id }})" 
                                    style="width: 100%; padding: 12px; background: #007bff; color: white; border: none; border-radius: 8px; font-size: 16px; font-weight: 600; cursor: pointer; transition: background 0.3s;"
            ...
            function startCountdown() {
                @if($featuredBundle->ends_at)
                    // Get end time from admin settings
                    const endTime = new Date('{{ $featuredBundle->ends_at->toISOString() }}');
                    ...
                    function updateTimer() {
                        ...
                        if (totalSeconds <= 0) {
                            // Timer expired
                            document.querySelector('.countdown-timer').innerHTML = '<div style="color: #dc3545; font-weight: bold;">OFFER EXPIRED</div>';
                        ...
                        // Show/hide days container based on time remaining
                        if (days > 0) {
    ...
    <section class="skc-section">
        <div class="skc-container">
            <div class="skc-section-header">
                <h2 class="skc-section-title">Shop by Category</h2>
                <p class="skc-section-subtitle">Discover our freshly baked delights</p>
            </div>
            ...
            <div class="swiper categories-swiper">
                <div class="swiper-wrapper">
                    ...
                    <div class="swiper-slide">
                        <a href="{{ route('products', ['category' => $category->slug]) }}" class="skc-category-card">
                            <div class="skc-category-icon">
                                @if($category->image_url && $category->image_url !== asset('img/placeholder-category.jpg'))
                                    <img src="{{ $category->image_url }}" alt="{{ $category->name }}" style="width: 64px; height: 64px; object-fit: cover; border-radius: 50%;">
                                @else
                                    @php
                                        $categoryEmojis = [
                                            'cakes' => '🎂',
                            ...
                            <h3 class="skc-category-name">{{ $category->name }}</h3>
                            <p class="skc-category-count">{{ $category->products_count ?? 0 }} Products</p>
                        </a>
                    </div>
                    @endforeach
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (heroSlides.length > 1) {
                // Pause on hover for the entire hero section
                const heroSection = document.querySelector('.skc-hero-section');
                if (heroSection) {
